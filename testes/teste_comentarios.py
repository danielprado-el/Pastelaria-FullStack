...
class TesteComentarios(unittest.TestCase):
    ...
    def testar_01_lazy_loading(self):
        ...
        for comentario in comentarios:
            self.assertEqual(comentario['pastel_id'], 1)

    def testar_02_enviar_comentario(self):
        nome = urllib.parse.quote("Anônimo")
        comentario = urllib.parse.quote(NOVO_COMENTARIO)

        resposta = self.enviar(f"{URL_ADICIONAR_COMENTARIO}/1/{nome}/anonimo@gmail.com/{comentario}", "POST")
